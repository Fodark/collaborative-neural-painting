...
class MaskGitModule(LightningModule):
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        ...
        pred_out_root = "/data/shared/ndallasen/datasets/inp/awesome-animals-complete/user-study-pred-maskgit/"
        ...
        my_little_datapoint = torch.cat(padded_levels).unsqueeze(0).to(self.device)
        my_little_datapoint = my_little_datapoint.half() if self.device == "cuda" else my_little_datapoint
        my_little_datapoint = my_little_datapoint.clamp(0, 1)
        my_little_datapoint *= 255
        my_little_datapoint = my_little_datapoint.long()
        my_little_datapoint = rearrange(my_little_datapoint, "b t c -> b (t c)")
        # my_little_datapoint = (my_little_datapoint * 255).long()
        ...
        for mode_ in ["block", "random", "level", "square"]:
            ctx, presence = self.mask_sequence(my_little_datapoint, mode_, deterministic=True)
            fake = self.sample_good(ctx, class_)
            # fake.clamp_(-self.scale // 2, self.scale // 2)
            fake = self.unnormalize(fake)
            ctx = self.unnormalize(ctx)

            ctx = rearrange(ctx, "b (s d) -> b s d", d=8)
            fake = rearrange(fake, "b (s d) -> b s d", d=8)
            fake = fake.clamp(0, 1)

            # ctx = torch.where(
            #     presence.unsqueeze(-1), torch.zeros_like(ctx), ctx
            # )
            ...
            rendered_fake = big_renderer.draw_on_canvas(fake + 1e-3, "black", False)[0]
