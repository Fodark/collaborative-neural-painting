...
def get_renderer_loaders(args):
    ...
    dataloaders = {
        x: DataLoader(datasets[x], batch_size=args.batch_size, shuffle=True, num_workers=4)
        for x in ["train", "val"]
    }
...
def img2patches(img, m_grid, s, to_tensor=True):
    ...
    for y_id in range(m_grid):
        for x_id in range(m_grid):
            patch = img[y_id * s : y_id * s + s, x_id * s : x_id * s + s, :].transpose([2, 0, 1])
...
def patches2img(img_batch, m_grid, to_numpy=True):
    ...
    for y_id in range(m_grid):
        for x_id in range(m_grid):
            ...
            img[y_id * s : y_id * s + s, x_id * s : x_id * s + s, :] = patch.permute([1, 2, 0])
    if to_numpy:
