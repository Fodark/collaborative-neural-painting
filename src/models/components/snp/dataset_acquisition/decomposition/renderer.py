...
class Renderer:
    def __init__(
        ...
        if self.renderer in ["markerpen"]:
            ...
            self.brush_small_vertical = cv2.imread(
                brush_paths["small_vertical"], cv2.IMREAD_GRAYSCALE
            )
            self.brush_small_horizontal = cv2.imread(
                brush_paths["small_horizontal"], cv2.IMREAD_GRAYSCALE
            )
            self.brush_large_vertical = cv2.imread(
                brush_paths["large_vertical"], cv2.IMREAD_GRAYSCALE
            )
            self.brush_large_horizontal = cv2.imread(
                brush_paths["large_horizontal"], cv2.IMREAD_GRAYSCALE
            )
    ...
    def create_empty_canvas(self):
        if self.canvas_color == "white":
            self.canvas = np.ones([self.CANVAS_WIDTH, self.CANVAS_WIDTH, 3]).astype("float32")
        else:
            self.canvas = np.zeros([self.CANVAS_WIDTH, self.CANVAS_WIDTH, 3]).astype("float32")
    ...
    def _draw_watercolor(self):
        ...
        self.foreground = np.zeros_like(self.canvas, dtype=np.uint8)  # uint8 for antialiasing
        ...
        self.stroke_alpha_map = np.array(self.stroke_alpha_map, dtype=np.float32) / 255.0
    ...
    def _draw_oilpaintbrush(self):
        ...
        # xc, yc, w, h, theta, R0, G0, B0, R2, G2, B2, A
        x0, y0, w, h, theta = self.stroke_params[0:5]
        R0, G0, B0 = self.stroke_params[5:8]  # remove ALPHA
        R2, G2, B2 = R0, G0, B0
        x0 = _normalize(x0, self.CANVAS_WIDTH)
        y0 = _normalize(y0, self.CANVAS_WIDTH)
        ...
        if self.highlight_border:
            cnt = cv2.findContours(
                self.stroke_alpha_map[:, :, 0][:, :, None].astype("uint8"),
                cv2.RETR_EXTERNAL,
        ...
        return np.expand_dims(self.stroke_alpha_map[:, :, 0], 0).astype(
            "bool"
        )  # return the alpha matte, only change

    def _update_canvas(self):
        return self.foreground * self.stroke_alpha_map + self.canvas * (1 - self.stroke_alpha_map)
